...
Bitrix24 (self-hosted) ─webhooks┼─► http://10.0.0.111  (Caddy, без TLS — внутр. сеть)
                                │        └─► api:8080 → worker + postgres + redis
сервер ─REST(outbound)──────────┴─► GitLab / Bitrix / LLM
```

!!! warning "HTTP без шифрования — только для изолированной сети"
    В этой конфигурации вебхуки идут по `http://` без TLS: секрет вебхука и тело запроса
    передаются **открытым текстом**. Применять только в доверенном изолированном сегменте.
    Если сегмент не изолирован — верните self-signed TLS (см. шаг 3) и доверьте корневой CA
    Caddy на сервере Bitrix. Аутентификация вебхуков по токену работает в обоих вариантах.
...
## 3. Прокси по HTTP (без TLS)

`Caddyfile` настроен на `http://10.0.0.111` с `bind 0.0.0.0` — Caddy слушает `:80` без
сертификата. Если IP другой — поправьте адрес сайта в `Caddyfile`.

Чтобы включить шифрование (self-signed TLS по IP), в `Caddyfile` раскомментируйте блок
`default_sni`, замените адрес на `https://10.0.0.111` и добавьте `tls internal`; тогда на
сервере Bitrix нужно доверить корневой CA Caddy:
`docker compose ... exec proxy cat /data/caddy/pki/authorities/local/root.crt`. Для домена —
`example.com` + автоматический ACME (без `bind`/`tls internal`).
...
`postgres`/`redis` закрыты в internal-сети; наружу смотрит только Caddy (порт `80`).

## 5. Зарегистрировать вебхуки

- **GitLab** (Settings → Webhooks): URL `http://10.0.0.111/gitlab-webhook`, Secret token =
...
- **Bitrix24** (Разработчикам → Исходящий вебхук): обработчик
  `http://10.0.0.111/bitrix-webhook`, события `ONTASKADD`, `ONTASKUPDATE`,
  `ONTASKCOMMENTADD`; `application_token` = значение `secrets/bitrix_app_token`.

!!! note "HTTP убирает возню с сертификатом"
    По `http://` ни проверка SSL, ни доверие к CA не нужны — поэтому self-signed-проблем на
    стороне Bitrix нет. Плата — секрет вебхука идёт открытым текстом (см. предупреждение
    выше). GitLab по-прежнему блокирует приватные IP: разрешите `10.0.0.111` в
    **Admin → Settings → Network → Outbound requests** (это не зависит от http/https).

## 6. Проверка

```bash
curl http://10.0.0.111/healthz     # {"status":"ok"}
curl http://10.0.0.111/readyz      # ready: true (Redis + Postgres)
...
на уровне фаервола: разрешите вход на `80` только с адресов GitLab и сервера Bitrix.
Без TLS это особенно важно — фаервол остаётся единственной сетевой границей.

```bash
# пример (ufw): только эти источники к 80
ufw allow from <GITLAB_IP> to any port 80 proto tcp
ufw allow from <BITRIX_IP> to any port 80 proto tcp
ufw deny 80/tcp
```
