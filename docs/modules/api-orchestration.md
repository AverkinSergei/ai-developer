...
- `run_self_fix` — реакция на упавший CI / `@ai fix`: находит задачу по `mr_iid`, чекаутит
  `source_branch` (ветку `auto-task-*`), запускает `coding.run_fix_loop` (прогон проверок репо
  → LLM-правки → повтор до зелёного либо лимита) и коммитит исправления обратно в ветку. При
  исчерпании лимита — fail-closed: правки не коммитятся, задача уходит человеку;
- `run_conflict_resolution` — реакция на `@ai resolve`: **честный ограниченный хендлер**.
  Полное 3-way-слияние отложено (чекаут хранится архивом, не git-репозиторием), поэтому
  конфликт маршрутизируется человеку с диагностикой, а не «молча» закрывается.
