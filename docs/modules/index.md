...
| Брифинг и авторизация | `briefing`, `briefing_store`, `briefing_state_machine`, `answer_extractor`, `go_authorizer`, `commands`, `intake`, `repo_planner` | [Брифинг](briefing.md) |
| Риск, план, код | `risk`, `context_engine`, `planning`, `coding` | [Риск, план, код](coding.md) |
| Гейты | `reviewer`, `redteam`, `gates`, `scope`, `ci`, `dod`, `preview` | [Гейты](gates.md) |
...
webhooks ──► worker ──► orchestrator ──┬──► briefing/* (intake, брифинг, /go)
                                       └──► planning ─► coding ─► gitlab_client (Draft MR)
                                                          │
                              reviewer / redteam / gates / scope / ci / dod / preview
```
